...
class DeepFeedForward(object):
    ...
    def __init__(self,
        ...
        self.input_size = len(self.dataframe.columns) - 1 #minus 1 since our target is one of the column 
    ...
    def predict(self,prediction_time=0,simple_mode=False):
        ...
        self.scaler.fit(df_to_fit,y=df_to_fit[self.prediction_target])
        ...
        target_index = df.columns.get_loc(self.prediction_target)
    ...
    def extract_train_df(self,scale=True):
        ...
        if scale is True:
            ...
            df = self.scaler.fit_transform(df,y=df[self.prediction_target])
            train = df[:-split,:]
            test = df[-split:,:]
        else:
